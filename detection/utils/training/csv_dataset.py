...
def _load_labels(label_path, shape, bb):
...
# Process labels stored in csv napari style.
# I don't actually think that we need the epsilon here, but will leave it for now.
def _process_labels(coords, shape, sigma, eps, bb=None):
...
def _load_image_data(raw_path, raw_key):
    if raw_key is None:
        return imageio.imread(raw_path)
    else:
        return open_file(raw_path, mode="r")[raw_key]
...
class CsvDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.raw = _load_image_data(self.raw_path, self.raw_key)
    ...
    def _get_sample(self, index):
        ...
        coords, n_points = _load_labels(label_path, shape, bb)
        ...
        label = _process_labels(coords, shape, self.sigma, self.eps, bb=bb)
